...
export default function Map({ selectedName, setSelectedName, isDarkMode }) {
	...
	function changeSelectedName(e) {
		let country;
		if (e.target.tagName === "path") {
			if (e.target.hasAttribute("name")) {
				country = e.target.getAttribute("name");
			} else {
	...
	function changeHoverName(e) {
		if (e.target.tagName === "path") {
			if (e.target.hasAttribute("name")) {
				setHoverName(e.target.getAttribute("name"));
			} else {
				setHoverName(e.target.classList.value);
			}
		} else {
			setHoverName("");
		}
	...
	return (
		<div className="map">
			{selectedName === "" && !isMobile && (
				<span
					className={`font-courier fixed p-[10px] tracking-[1px] transition-all duration-600 z-100 ${
						!isDarkMode ? "text-black font-extrabold" : "text-white"
					} no-select`}
				>
					{hoverName}
				</span>
			...
			<svg
				className={`${
					isDarkMode ? "stroke-black" : "stroke-white"
				} w-fit h-fit pt-[60px] pl-[35px] transition-transform duration-800 ease-in-out`}
				onClick={changeSelectedName}
				onMouseOver={changeHoverName}
				baseprofile="tiny"
				height="600"
			...
			>
				<path
					className={`${
						isDarkMode
							? "stroke-[0.3] stroke-white fill-[#022939] hover:fill-[#028fbe]"
							: "stroke-[0.9] stroke-black fill-[#c4e4e8] hover:fill-[#13708f]"
					} cursor-pointer hover:appear-effect scale-70`}
